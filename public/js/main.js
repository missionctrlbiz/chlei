; (function ($) {
  ...
  $(window).on("load", function (event) {
    // Preloader behavior is handled by the React `Loader` component.
    // Removed jQuery fade toggle to avoid conflicts with React-managed preloader.
    ...
    // Add a runtime fallback for missing icon images to avoid 404s breaking layout
    try {
      document.querySelectorAll('img[src^="/img/icons/"]').forEach(function (img) {
        img.addEventListener('error', function () {
          // prevent infinite loop if fallback missing
          if (this.dataset.fallbackSet) return;
          this.dataset.fallbackSet = '1';
          this.src = '/img/icons/vl-service-heart.svg';
        });
      });
